...
class ball:
    def __init__(self, canvas, bar):
        self.canvas = canvas
        self.bar = bar
        self.id = canvas.create_oval(0, 0, 15, 15, fill = "red")
...
class bar:
    def __init__(self, canvas):
        self.canvas = canvas
        self.id = canvas.create_rectangle(0, 0, length, 10, fill = "black")
...
def score():
    canvas.itemconfig(score_info, text = "Pontos: " + str(count))
...
score_info = canvas.create_text(430, 20, text = "Pontos: " + str(count), fill = "green", font = ("Arial", 16))
